...
def main():
	...
	uriBox.insert(END, 'https://sparql.vanderbilt.edu/sparql')
	commandBox = ttk.Entry(root, width = 120, textvariable = StringVar())
	commandBox.insert(END, 'LOAD <http://vuswwg.jelastic.servint.net/gom/dump/occurrenceStatus> INTO GRAPH <http://example.org/occ>')
	...
	def buttonClick():
		...
		r = requests.post(uriBox.get(), auth=(usernameBox.get(), passwordBox.get()), data = {'update': commandBox.get()} )
