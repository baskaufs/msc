...
# read in the CSV file of still unassociated records
path = "unassociated.csv"
...
for row in identifiedData:
    if rowNum != 0:
        identified.append(row[1])  #in this file, the essss numbers are in the second column
    rowNum += 1

# Open the CSV file that will contain the data not yet associated with essss numbers
unassociatedFileObject = open("orphaned.csv", 'w', newline='')
