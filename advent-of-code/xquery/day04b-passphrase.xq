...
declare function functx:alphachars
...
declare function local:sortletters($password as xs:string) as xs:string
{
  string-join(functx:alphachars($password))
};

declare function local:convert-sequence($seq as xs:string+) as xs:string+
{
  for $word in $seq
  return local:sortletters($word)
};

let $puzzle := "nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
let $bools :=
    for $phrase in $lines
    let $sequence := local:convert-sequence(tokenize($phrase))
    let $distinct := count(distinct-values($sequence))
    let $words := count($sequence)
    return $words=$distinct
return count($bools[. = true()])
